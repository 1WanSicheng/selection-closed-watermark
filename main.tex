...
preservation alone controls nothing --- and show that the repair requires
nothing new: \emph{standard cryptographic domain separation} --- per-candidate
subkeys, the jointly-coupled transform $\mathrm{JC}_N$ --- restores closure
completely and minimally for any keyed scheme: \emph{exactly}, if the $N$
random keys are stored independently; \emph{computationally}, under a compact
PRF-derived master key. The scheme's own detector lifts by a union wrapper
whose cost is a once-per-sequence $\sqrt{2\ln N}$ threshold shift, and the
guarantees are scheme- and selector-agnostic. Instantiated on the deployed
...
LLaMA-3.1-8B-Instruct) with each candidate's marginal guarantee unchanged,
retains detection at $\approx0.3$ z-score cost, and dominates the deployed
...
exact marginal certification it was meant to protect, and we show it is in fact
...
contribution is the missing deployment-level property and the proof that this
...
(Theorem~\ref{thm:lifting}), for every selection operator.
...
\begin{enumerate}
  ...
  \item Instantiating the framework on the deployed Gumbel/Aaronson scheme, a
        real aligned model shows full recovery of the BoN gain with the
        candidate marginal unchanged, and dominance over the deployed softened
...
$\mu_W$.
(a) \emph{Exact tier (independently stored keys):} if the $N$ candidates are
driven by $N$ independently sampled and stored random keys, their joint law
\emph{equals} $\mu_W^{\otimes N}$ --- total variation exactly zero --- for any
base scheme $W$, distortion-free or not.
(b) \emph{Computational tier (PRF-derived subkeys):} with subkeys derived from
one master key by a PRF ($\mathrm{JC}_N$), the joint is computationally
...
is $N$-selection-closed (exactly in the stored-keys tier, up to
$\varepsilon$ in the PRF tier), and by Theorem~\ref{thm:closure} the released
output under
...
inherits the base scheme's single-sample guarantee unchanged.
...
the entire BoN gain; subkeys restore the unwatermarked ceiling with the
candidate marginal unchanged; union detection costs $\approx0.3$ z. The softened workaround
...
reward $0.0698\approx0.0703$ (candidate marginal unchanged), hard-BoN
...
nothing), and preserves the exact marginal certification where softening does
not.
...
simple ($N$-selection-closedness), and the remedy is standard domain separation, deliberately unoriginal:
$\mathrm{JC}_N$ lifts any keyed scheme --- existing detectors included --- to
joint closure (exact with independently stored keys, computational under one
compact master key), at a once-per-sequence $\sqrt{2\ln N}$ detection cost. Watermarks should be
...
\textbf{(a) Exact tier.} With $N$ independently sampled and stored random keys
the candidates are
...
\textbf{(b) Computational tier.} All subkeys are evaluations of one PRF at
distinct points
