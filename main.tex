...
Test-time alignment increasingly improves language-model outputs by generating
multiple candidates and selecting among them: Best-of-$N$ selects by a reward
model \citep{verma2025watermarking}, verifier-guided inference selects a
verified solution, rejection sampling filters low-quality outputs, and
self-consistency aggregates several reasoning traces. These methods obtain
their gain from candidate diversity and from the joint structure of the
candidate pool. Distortion-free watermarking, by contrast, is certified one
generation at a time: averaged over the secret key, a \emph{single} watermarked
sample follows the same law as the unwatermarked model
\citep{aaronson2023, kuditipudi2023robust, zhao2024permute}. These two views do
not automatically compose --- a marginal guarantee for each candidate does not
determine the distribution produced after a selector observes all candidates.
This paper asks:
\begin{center}
\emph{When does a distortion-free watermark compose correctly with
Best-of-$N$ and other test-time selectors?}
\end{center}
We show that for the deterministic context-hashed scheme family commonly
studied and used in current watermarking systems, it does not --- the entire
test-time alignment gain can vanish --- and that a minimal lift restores it.
...
\textbf{From candidate diversity to selection correctness.} It is well known
that deterministic decoding with reused randomness reduces candidate diversity
\citep{verma2025watermarking}. Our focus is different: we ask whether the
\emph{distributional guarantee} of a distortion-free watermark survives a
downstream selector, which requires reasoning about the joint candidate law,
not only about whether candidates happen to be distinct. We show that (i)
exact marginals can still yield a biased post-selection distribution, with the
worst-case shift approaching $\mathrm{TV}=\tfrac12$; (ii) joint preservation
is exactly the condition needed for selector-universal closure; and (iii)
candidate-separated randomness restores the reference behavior for
Best-of-$N$, reranking, rejection sampling, and other key-independent
selectors, at an order-optimal detection cost.

\textbf{Why this is not merely a seeding issue.} Assigning different
randomness to different candidates is an operationally natural fix, but it
does not answer the underlying question: \emph{what property must a
watermarked candidate pool satisfy so that an arbitrary inference-time
selector behaves as it would on unwatermarked samples?} The issue persists
beyond complete stream reuse --- any dependence among candidates can alter the
output of a reward-based selector while leaving every marginal unchanged
(Lemma~\ref{lem:attenuation} quantifies the attenuation). Our contribution is
to identify and characterize the required joint guarantee;
candidate-specific domain separation is then the minimal construction that
realizes it.
...
converts joint correlation into first-order bias of the \emph{post-selection output}.
...
watermarked models and explicitly sacrifices exact marginal
distortion-freeness for
...
computationally; the construction is not practical to instantiate. Our notion is
the minimal targeted property, and our lift upgrades the context-hash family in
place
(Proposition~\ref{prop:hierarchy}).
