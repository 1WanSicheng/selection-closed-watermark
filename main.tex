...
\title{Distortion-Free Until You Select: A General Framework for\\
Jointly-Coupled Watermarking under Inference-Time Selection}
...
preservation is sufficient for \emph{any} selection operator, and give a
...
log. For max-stable score families (Gumbel/exponential) we additionally analyze
a column-max \emph{aggregate} detector (the right fallback when $N$ is unknown
or outputs are block-composed): its null is \emph{exactly} calibrated, but its
per-token signal contracts in closed form from $\Delta$ to
$\ln(1+(e^{\Delta}-1)/N)$ (Proposition~\ref{prop:aggregate}, verified to
machine precision, Figure~\ref{fig:detectors}) --- a per-token $\ln N$ dilution
that collapses in the low-entropy regime. The two detectors are thus separated
by a per-sequence $\sqrt{2\ln N}$ versus a per-token $\ln N$.
...
\begin{enumerate}
  ...
  \item We give the general jointly-coupled transform $\mathrm{JC}_N$ --- a
        ...
        (Theorem~\ref{thm:lifting}) --- together with a lifted detection theory:
        the scheme-agnostic union wrapper vs.\ the family-specific aggregate
        detector, separated by a per-sequence $\sqrt{2\ln N}$ vs.\ per-token
        $\log N$ cost (\S\ref{sec:method}).
...
\citep{fernandez2023three}; our union wrapper inherits this standard practice, and
...
A \emph{keyed sampler} $W$ is any watermarked decoder whose per-step randomness
is a deterministic (pseudorandom) function of a key $k$ and the generation
state, e.g., context-hash Gumbel/Aaronson, exponential/inverse-transform
sampling, tournament sampling, or biased-logit schemes. The
\emph{jointly-coupled lift} $\mathrm{JC}_N(W)$ generates candidate
$i\in[N]$ by running $W$ \emph{unmodified} under the domain-separated subkey
$\mathrm{SubKey}_i=\mathrm{PRF}(\mathrm{MasterKey},\,i)$. One master key is
stored; the derivation is free.

\begin{theorem}[Lifting]\label{thm:lifting}
Let $W$ be a keyed sampler with single-sample (key-averaged) output law $\mu_W$.
Then, under PRF security:
(a) the $N$ candidates of $\mathrm{JC}_N(W)$ are computationally
indistinguishable from $N$ i.i.d.\ draws of $\mu_W$ --- for \emph{any} base
scheme $W$, distortion-free or not;
(b) if $W$ is marginally distortion-free ($\mu_W=Q$), then $\mathrm{JC}_N(W)$
is $N$-selection-closed, and by Theorem~\ref{thm:closure} the released output
under \emph{any} selection operator is distributed as unwatermarked
Best-of-$N$;
(c) any detector for $W$ lifts to a detector for $\mathrm{JC}_N(W)$ (the union
wrapper of \S\ref{sec:detection}) with a once-per-sequence multiplicity cost.
\end{theorem}
\begin{proof}[Proof sketch]
(a) Domain separation makes the $N$ noise streams jointly indistinguishable
from independent streams; each candidate is then an independent run of $W$,
whose output law is $\mu_W$. (b) i.i.d.\ with marginal $Q$ is precisely
Definition~\ref{def:closure}. (c) is Proposition~\ref{prop:union}. Full proof
in Appendix~\ref{app:proofs}. \qedhere
\end{proof}

Part (a) is worth emphasizing: even for schemes that are \emph{not}
distortion-free (e.g., biased-logit families), the lift restores candidate
i.i.d.-ness, so selection behaves as ordinary Best-of-$N$ over the scheme's
marginal --- removing the selection-induced pathology while leaving the
scheme's own marginal bias unchanged. For distortion-free $W$, part (b) is the
full closure guarantee. As a byproduct, strict (argmax) decoders can be used
again --- no softening --- so each candidate individually retains the exact
...
scheme's own detector under each derived subkey --- while the \emph{aggregate}
variant exploits max-stability and is specific to Gumbel-family scores, which we
adopt below for concreteness. We analyze both under a location-shift signal
model:
...
$\Gamma$-null detector; the aggregate detector applies since the score family
is max-stable.
\emph{Exponential / permute-and-flip samplers} \citep{zhao2024permute}: identical
treatment --- the noise is the same exponential family, so both detectors carry
over verbatim.
...
selectors (including rejection sampling and min-perplexity reranking) and two
...
\textbf{(a)} All subkeys are evaluations of one PRF at distinct points
...
therefore distinguish the streams, contradicting PRF security. Note that (a)
makes no assumption on $W$ beyond keyed-ness --- in particular no
distortion-freeness.
\textbf{(b)} If $\mu_W=Q_x$ for every prompt, then i.i.d.\ with marginal $Q_x$
is precisely Definition~\ref{def:closure}, and Theorem~\ref{thm:closure} yields
closure under every selection operator.
\textbf{(c)} The base detector's null is exact under each wrong subkey (the
...
single, uniform-random, hard-/soft-/Exp-BoN, rejection sampling (accept the
first candidate with reward above the unwatermarked median, random scan order),
and min-perplexity reranking --- and two \emph{key-dependent} stress selectors
...
min-PPL       & yes & $-0.0060$ & $-0.0010$ & $-0.0029$ & $5.16$ & $0.828$ \\
\midrule
...
operating point. (v) \emph{Quality reranking} (min-PPL) is the most
watermark-hostile benign selector ($z$ $5.16$): the most model-typical
candidate carries the weakest signal. Consistently, the generator's
likelihood-sorted candidate order exhibits a monotone gradient --- reward
falls from $0.073$ to $0.059$ and median $z$ rises from $4.7$ to $12.9$ across
likelihood ranks --- i.e., model-typical text is simultaneously
reward-preferred and watermark-poor, the within-set mirror of the
reward--watermark anticorrelation.
