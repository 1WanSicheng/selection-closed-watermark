...
class OpenaiGeneratorBeamStrict(WmGeneratorBeam):
    """Strict (argmax) Aaronson watermark for BoN.
    subkey=False: shared key  -> all candidates of a prompt are IDENTICAL (deterministic).
    subkey=True : per-candidate domain-separated subkey -> candidates are iid true nucleus samples
                  (strict distortion-free marginal AND independent joints)."""
...
class PFGeneratorBeam(WmGeneratorBeam):
    """PF (permute-and-flip / exponential-noise) watermark, softened for beam/BoN diversity.

    argmax(log p - log r) is PF; here we sample multinomial from p/r = exp(log p - log r)
    (single key via self.rng-seeded rs; diversity from the multinomial), mirroring
    OpenaiGeneratorBeam's r^(1/p) softening. Watermark signal: chosen token biased to small r.
    """
    ...
    def sample_next(
        ...
        if temperature > 0:
            ...
            # work in log-space: score = log p - log r (masked -> -inf); softmax before multinomial
            # avoids inf/nan from raw p/r when r is tiny (Gumbel's r^(1/p) is bounded, PF's p/r is not)
            ...
            for ii in range(ngram_tokens.shape[0]):
                ...
                rs = torch.rand(vocab_size, generator=self.rng, device=probs_sort.device)
                rs = rs.roll(-self.payload)
                rs = rs[probs_idx[ii]].clamp_min(1e-9)
                log_probs[ii] = log_probs[ii] - rs.log()

            # robust softmax in fp32 (fp16 can overflow exp); bulletproof to nan/inf/degenerate
            lp = torch.nan_to_num(log_probs.float(), nan=-1e30, posinf=30.0, neginf=-1e30)
            soft = torch.softmax(lp, dim=-1)
            soft = torch.nan_to_num(soft, nan=0.0, posinf=0.0, neginf=0.0).clamp_min(0.0) + 1e-8
            next_tokens = torch.multinomial(soft, num_samples=1, generator=self.rng)
            next_scores = torch.gather(soft, -1, next_tokens)
            next_tokens = torch.gather(probs_idx, -1, next_tokens)
        else:
            next_scores, next_tokens = torch.max(logits, dim=-1, keepdim=True)
